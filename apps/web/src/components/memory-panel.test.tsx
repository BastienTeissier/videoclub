...
describe("MemoryPanel", () => {
  ...
  it("renders all four rows when the snapshot has every field", () => {
    ...
    // DOM order is source order (newest last); visual reversal is via
    // flex-col-reverse so React keys stay stable when notes append.
    const notes = screen.getAllByRole("listitem");
    expect(notes).toHaveLength(2);
    expect(notes[0]!).toHaveTextContent("one");
    expect(notes[1]!).toHaveTextContent("two");
  });
  ...
  it("flashes the row whose key is in flashingKeys", () => {
    ...
    expect(rowDiv.className).toContain("bg-commitment/15");
    expect(rowDiv.className).toContain("ring-commitment/40");
  });
  ...
  it("does not flash when only applySnapshot is called", () => {
    ...
    const rowDt = screen.getByText("Genres");
    const rowDiv = rowDt.parentElement!;
    expect(rowDiv.className).not.toContain("bg-commitment/15");
  });
});
